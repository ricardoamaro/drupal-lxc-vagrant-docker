...
nginx::resource::location { 'drupal.dev-php':
  ensure              => 'present',
  vhost               => 'drupal',
  location            => '~ \.php$',
  proxy               => undef,
  ...
  require             => Nginx::Resource::Vhost['drupal'],
}
...
nginx::resource::location { "xhprof-php":
  ensure              => 'present',
  vhost               => 'xhprof',
  location            => '~ \.php$',
  proxy               => undef,
  ...
  notify              => Class['nginx::service'],
  require             => Nginx::Resource::Vhost['xhprof'],
}
...
class { 'mysql::server':
  config_hash   => { 'root_password' => 'puppetdrupal' }
}
